...
instance Ord ExprTree where
    ...
    compare (Div (Const _) (Const _)) _ = LT
    compare _ (Div (Const _) (Const _)) = GT  -- first term is definetely not Const
    compare (Var _) _           = LT
    compare _ (Var _)           = GT
...
makeCanonical expr@(Mult [x, p@(Pow (Var _) (Const _))]) = 
    if isConstant x 
...
transformNegatives (Sub (Const a) op)  = Add [Const a, Mult [Const $ -1, transformNegatives op]]
...
transformNegatives (Add opLst)         = Add $ map transformNegatives opLst
transformNegatives (Mult opLst)        = Mult $ map transformNegatives opLst
transformNegatives (Pow x y)           = Pow $ transformNegatives opLst
transformNegatives exprTree            = exprTree
...
simplify expr = if simplified == expr
                then expr
